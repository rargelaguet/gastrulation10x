...
p$add_argument('--stageA',    type="character",    help='group A')
p$add_argument('--stageB',    type="character",    help='group B')
p$add_argument('--celltype',    type="character",    nargs="+", help='Celltype to use')
...
# args$stageA <- c("E6.5")
# args$stageB <- c("E7.0")
# args$celltype <- c("Epiblast")
# args$outfile <- c("/Users/ricard/data/gastrulation10x/results/differential/stages/foo.tsv.gz")
...
# Sanity checks
stopifnot(args$celltype%in%opts$celltypes)
stopifnot(args$stageA%in%opts$stages)
stopifnot(args$stageB%in%opts$stages)
...
# Define groups
opts$groups <- c(args$stageA,args$stageB)
...
sample_metadata <- sample_metadata %>%
  .[stage%in%opts$groups & celltype==args$celltype] %>%
  setnames("stage","group") %>%
  .[,c("cell","celltype","group")]

# Sort cells so that stageA comes before stageB
...
) %>% setnames(c("ens_id","detection_rate_groupA","detection_rate_groupB"))

# Filter genes
sce <- sce[rownames(sce)%in%gene_metadata$ens_id,]

# Remove houseekeping genes that are expressed everywhere and genes that are not expressed anywhere
cdr.dt <- cdr.dt %>%
  .[!(detection_rate_groupA<0.01 & detection_rate_groupB<0.01)] %>%
  .[!(detection_rate_groupA>0.99 & detection_rate_groupB>0.99)]
sce <- sce[rownames(sce) %in% cdr.dt$ens_id]
