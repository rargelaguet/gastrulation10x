...
if (grepl("ricard",Sys.info()['nodename'])) {
  ...
  io$tmpdir <- paste0(io$basedir,"/results/differential/stages/tmp"); dir.create(io$tmpdir, showWarnings=F)
} else {
...
for (i in opts$celltypes) {
  ...
  for (j in 1:(length(opts$stages)-1)) {
    stageA <- opts$stages[[j]]
    for (k in (j+1):length(opts$stages)) {
